...
import {Eip4337Configuration} from "src/scripts/reflections/di/configurations/Eip4337Configuration.s.sol";
...
contract StdConfigBasedWiring is IWiringMechanism, Config {
    ...
    function wire(
        ...
        if (wiringType == SupportedWiring.PLAIN) {
            ...
            address wiredAddress = Create2.deploy(
                0,
                source.toSalt(),
                source.toCreationCode()
            );
            ...
            if (wiringInfo.length >= 64) {
                ...
                if (stdConfigurationFlag == Sources.NICKNAMED_PROXY_FLAG) {
                    ...
                    return
                        config.get(tupConfig.getProxySourceKey()).toAddress();
                } else if (stdConfigurationFlag == Sources.EIP4337_FLAG) {
                    (address ownerAddress, bytes memory restOfInfo) = abi
                        .decode(deployerAndRestOfInfo, (address, bytes));
                    if (ownerAddress == address(0)) {
                        revert UnsupportedWiringType();
                    }
                    ShortString nickname = ShortStrings.toShortString(
                        abi.decode(restOfInfo, (string))
                    );
                    Eip4337Configuration eip4337Config = new Eip4337Configuration(
                            vm,
                            config,
                            ownerAddress,
                            nickname
                    ...
                    return
                        config
                            .get(eip4337Config.getAccountSourceKey())
                            .toAddress();
                } else {
                    revert UnsupportedWiringType();
                }
    ...
    function getWiredVariants(
        bytes memory wiringInfo,
        SupportedWiring wiringType
    ) external view virtual override returns (address[] memory result) {
        if (address(config) == address(0)) {
            revert ChooseConfigurationFirst();
        }
        ...
        if (wiringType == SupportedWiring.PLAIN) {
            ...
            string memory sourceKey = string.concat(
                string.concat(source.toString(), "_"),
                nickname.toString()
            );
