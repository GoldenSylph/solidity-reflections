...
abstract contract Autowirable is Script {
    ...
    modifier configwire(IConfiguration configContract) {
        ...
        wiringMechanism.wire(
            abi.encode(address(configContract)),
        ...
        console.log(
            ...
            configContract.name(),
            ") utilized:",
            Strings.toHexString(address(configContract))
        );
        _;
    }
    ...
    modifier autowire(Sources.Source source) {
        address injectedAddress = wiringMechanism.wire(
            abi.encode(uint256(source)),
            IWiringMechanism.SupportedWiring.PLAIN
        );
        console.log(
            string(
                abi.encodePacked(
    ...
    modifier proxywire(Sources.Source source) {
        ...
        address injectedAddress = wiringMechanism.wire(
            abi.encode(
                Sources.NICKNAMED_PROXY_FLAG,
                abi.encode(msg.sender, abi.encode(source))
            ),
            IWiringMechanism.SupportedWiring.CONFIGURATION_BASED
        );
        console.log(
    ...
    modifier accountwire(string memory nickname) {
        ...
        address injectedAddress = wiringMechanism.wire(
            abi.encode(
                Sources.EIP4337_FLAG,
                abi.encode(msg.sender, abi.encode(nickname))
            ),
            IWiringMechanism.SupportedWiring.CONFIGURATION_BASED
        );
        console.log(
    ...
    modifier nickwire(Sources.Source source, ShortString nickname) {
        address injectedAddress = wiringMechanism.wire(
            abi.encode(uint256(source), nickname),
    ...
    function autowired(
        ...
        address[] memory sortedInjectedAddresses = wiringMechanism
            .getWiredVariants(
                abi.encodePacked(uint256(source)),
                IWiringMechanism.SupportedWiring.PLAIN
            );
        if (
            sortedInjectedAddresses.length == 0 ||
            sortedInjectedAddresses[0] == address(0)
        ) {
    ...
    function autowired(
        ...
        address[] memory sortedInjectedAddresses = wiringMechanism
            .getWiredVariants(
                abi.encodePacked(
                    uint256(source),
                    ShortStrings.toShortString(nickname)
                ),
                IWiringMechanism.SupportedWiring.PLAIN_NICKNAMED
            );
        if (
