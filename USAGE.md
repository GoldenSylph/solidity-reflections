...
`Reflections` is straightforward to use. It can either be invoked from the `forge` tool provided by Foundry, or installed as
a standalone executable named `Reflections`.

Dependencies and configuration options can be specified inside Foundry's `foundry.toml` config file, or inside a
dedicated `Reflections.toml` file.
...
```bash
[forge] Reflections init [--clean]
```

The `init` command can be used to setup a project for use with Reflections. The command will generate or modify the
project's config file (`foundry.toml` or `Reflections.toml`) and perform optional removal of Foundry-style submodule
dependencies with the `--clean` flag.

This command automatically adds the latest `forge-std` dependency to your project.

Note that Reflections installs dependencies into a folder named `dependencies`. There is currently no way to customize this
path.

## Adding Dependencies

### From the Reflections Registry

```bash
[forge] Reflections install <NAME>~<VERSION>
```

This command searches the Reflections registry at [https://Reflections.xyz](https://Reflections.xyz) for the specified dependency
...
Note that this only makes sense when used with the Reflections registry, as it provides a list of available versions to
...
```bash
[forge] Reflections install <NAME>~<VERSION> --url <ZIP_URL>
```
...
```bash
[forge] Reflections install <NAME>~<VERSION> --git <GIT_URL>
```
...
```bash
[forge] Reflections install test-project~v1 --git git@github.com:test/test.git
[forge] Reflections install test-project~v1 --git git@gitlab.com:test/test.git
```

```bash
[forge] Reflections install test-project~v1 --git https://github.com/test/test.git
[forge] Reflections install test-project~v1 --git https://gitlab.com/test/test.git
```

```bash
[forge] Reflections install test-project~v1 --git git@github.com:test/test.git --rev 345e611cd84bfb4e62c583fa1886c1928bc1a464
[forge] Reflections install test-project~v1 --git git@github.com:test/test.git --branch dev
[forge] Reflections install test-project~v1 --git git@github.com:test/test.git --tag v1
```
...
```bash
[forge] Reflections install
```

When invoked without arguments, the `install` command installs the project's existing dependencies by looking at the
configuration file (`Reflections.toml`/`foundry.toml`) and lockfile `Reflections.lock` if present.
...
newer version exists on the registry. To update the lockfile to use the latest supported version, use `Reflections update`.

### Recursive Installation

With the `--recursive-deps` flag, Reflections will install the dependencies of each installed dependency, recursively.
This is done internally by running `git submodule update --init --recursive` and/or `Reflections install` inside of the
...
```bash
[forge] Reflections update
```

For dependencies from the online registry which specify a version range, the `update` command can be used to retrieve
the latest version that matches the requirements. The `Reflections.lock` lockfile is then updated accordingly. Remappings
are automatically updated to the new version if Reflections is configured to generate remappings.
...
```bash
[forge] Reflections uninstall <NAME>
```
...
```bash
[forge] Reflections push <NAME>~<VERSION>
```

In order to push a new dependency to the repository, an account must first be created at
[https://Reflections.xyz](https://Reflections.xyz). Then, a project with the dependency name must be created through the
website.

Finally, the `[forge] Reflections login` command must be used to retrieve or provide an access token for the API.
CLI tokens can be generated on Reflections.xyz and should be preferred over using the email and password in the CLI,
because email login will be removed in a future version of Reflections.
Alternatively, you can provide a valid CLI token via the `Reflections_API_TOKEN` environment variable.

Example:

Create a project called `my-project` and then use the `[forge] Reflections push my-project~1.0.0`. This will push the
project to the repository as version `1.0.0` and makes it available for anyone to use.

### Specifying a Path

```bash
[forge] Reflections push <NAME>~<VERSION> [PATH]
```
...
If you want to ignore certain files from the published package, you need to create one or more `.Reflectionsignore` files
...
```bash
[forge] Reflections push <NAME>~<VERSION> --dry-run
```
...
exclude sensitive files in the `.Reflectionsignore` or `.gitignore` file.
...
The `foundry.toml`/`Reflections.toml` file can have a `[Reflections]` section to configure the tool's behavior.

See the default configuration below:

```toml
[Reflections]
# whether Reflections manages remappings
remappings_generate = true

# whether Reflections re-generates all remappings when installing, updating or uninstalling deps
...
# ignored when `Reflections.toml` is used as config (uses `remappings.txt`)
...
For more commands and their usage, see `[forge] Reflections --help` and `[forge] Reflections <COMMAND> --help`.

## Remappings Caveats

If you use other dependency managers, such as git submodules or npm, ensure you don't duplicate dependencies between
Reflections and the other manager.

Remappings targeting dependencies installed without Reflections are not modified or removed when using Reflections commands,
unless the `--regenerate-remappings` flag is specified or the `remappings_regenerate = true` option is set.

## Dependencies Maintenance

The vision for Reflections is that major projects such as OpenZeppelin, Solady, Uniswap would start publishing their own
packages to the Reflections registry so that the community can easily include them and get timely updates.

Until this happens, the Reflections maintenance team (currently m4rio.eth) will push the most popular dependencies to the
repository by relying on their npmjs or GitHub versions. We are using
[an open-source crawler tool](https://github.com/mario-eth/Reflections-crawler) to crawl and push the dependencies under the
`Reflections` organization.

For those who want an extra layer of security, the `Reflections.lock` file saves a `SHA-256` hash for each downloaded
ZIP file and the corresponding unzipped folder (see `Reflections_core::utils::hash_folder` to see how it gets generated).
These can be compared with the official releases to ensure the files were not manipulated.

**For Project Maintainers**
If you want to move your project from the Reflections organization and take care of pushing the versions to Reflections
